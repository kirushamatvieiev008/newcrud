...
var _createitemdata = require("./api/createitemdata");
var _deluser = require("./api/deluser");
var _updateitemdata = require("./api/updateitemdata");
var _rendertemplate = require("./templates/rendertemplate");
...
btnGetStudents.addEventListener("click", ()=>{
    (0, _getdata.getData)().then((res)=>{
        usersList.innerHTML = (0, _rendertemplate.renderStudents)(res);
        isClicked = true;
    });
});
formAddtudents.addEventListener("submit", (event)=>{
    ...
    const userObject = {
        ...
        isEnrolled: event.currentTarget.elements.isEnrolled.checked ? "enroled" : "not enrolled"
    };
    // const zeroInps = () => {
    //   console.log(event.currentTarget);
    // }
    // console.log(event.currentTarget);
    (0, _createitemdata.createItem)(userObject).then(()=>{
        if (isClicked) (0, _getdata.getData)().then((res)=>{
            usersList.innerHTML = (0, _rendertemplate.renderStudents)(res);
        });
    // zeroInps();
    });
...
tablestudents.addEventListener("click", (event)=>{
    ...
    if (event.target.classList[0] === "delete") (0, _deluser.deleteUser)(userId).then(()=>{
        (0, _getdata.getData)().then((res)=>{
            usersList.innerHTML = (0, _rendertemplate.renderStudents)(res);
        });
    });
        ...
        updateForm.elements.isEnrolled.checked = elementsInp[6].textContent === "enrolled" ? true : false;
    }
});
updateForm.addEventListener("submit", (eventIns)=>{
    ...
    (0, _updateitemdata.update)(elementsInp[0].textContent, updatedUser).then(()=>{
        (0, _getdata.getData)().then((res)=>{
            usersList.innerHTML = (0, _rendertemplate.renderStudents)(res);
        });
    });
